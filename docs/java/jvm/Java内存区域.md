...
- [Java 内存区域详解](#java-内存区域详解)
    - [写在前面 (常见面试题)](#写在前面-常见面试题)
        ...
        - [拓展问题](#拓展问题)
    - [一 概述](#一-概述)
    - [x] [二 运行时数据区域](#二-运行时数据区域)
        - [x] [2.1 程序计数器](#21-程序计数器)
        - [x] [2.2 Java 虚拟机栈](#22-java-虚拟机栈)
        - [x] [2.3 本地方法栈](#23-本地方法栈)
        - [x] [2.4 堆](#24-堆)
        - [x] [2.5 方法区](#25-方法区)
            - [x] [2.5.1 方法区和永久代的关系](#251-方法区和永久代的关系)
            - [x] [2.5.2 常用参数](#252-常用参数)
            - [x] [2.5.3 为什么要将永久代 (PermGen) 替换为元空间 (MetaSpace) 呢?](#253-为什么要将永久代-permgen-替换为元空间-metaspace-呢)
        - [x] [2.6 运行时常量池](#26-运行时常量池)
        - [x] [2.7 直接内存](#27-直接内存)
    - [x] [三 HotSpot 虚拟机对象探秘](#三-hotspot-虚拟机对象探秘)
        - [x] [3.1 对象的创建](#31-对象的创建)
            - [x] [Step1:类加载检查](#step1类加载检查)
            - [x] [Step2:分配内存](#step2分配内存)
            - [x] [Step3:初始化零值](#step3初始化零值)
            - [x] [Step4:设置对象头](#step4设置对象头)
            - [x] [Step5:执行 init 方法](#step5执行-init-方法)
        - [x] [3.2 对象的内存布局](#32-对象的内存布局)
        - [x] [3.3 对象的访问定位](#33-对象的访问定位)
    - [x] [四  重点补充内容](#四--重点补充内容)
        - [x] [4.1 String 类和常量池](#41-string-类和常量池)
        - [x] [4.2 String s1 = new String("abc");这句话创建了几个字符串对象？](#42-string-s1--new-stringabc这句话创建了几个字符串对象)
